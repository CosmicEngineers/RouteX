...
export function EnhancedOptimizationPanel({ availableVessels, availablePorts: _availablePorts, onStartOptimization }: EnhancedOptimizationPanelProps) {
  ...
  const validateInputs = () => {
    ...
    if (totalDemand > totalCapacity * 2) {
      warnings.push(`Total demand (${totalDemand.toLocaleString()} MT) exceeds available fleet capacity. Additional vessels may be required.`);
    }

    if (fuelPrice < 35000 || fuelPrice > 55000) {
      warnings.push(`Fuel price ₹${fuelPrice.toLocaleString()} is outside the typical marine fuel range (₹35k-55k/MT). Please verify.`);
    }

    if (maxSolveTime < 60) {
      warnings.push('Optimization time under 60 seconds may not find the best solution. Consider using "Standard" speed.');
    }
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <div className="font-semibold text-amber-300 mb-2">Configuration Warnings:</div>
      ...
      <div className="glass-card rounded-xl border border-slate-700/50 p-6">
        ...
        <div className="space-y-3">
          ...
          <div className="text-xs text-slate-500 text-center">
            {totalDemand > totalCapacity
              ? `⚠️ Demand exceeds fleet capacity by ${(totalDemand - totalCapacity).toLocaleString()} MT - multiple trips required`
              : `✓ ${((totalCapacity - totalDemand) / 1000).toFixed(0)}k MT available capacity for additional deliveries`}
          </div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-xl border border-slate-700/50 p-6" id="optimization-panel">
        <div className="space-y-5">
          ...
          <div>
            ...
            >
              <option value="cost">Lowest Transportation Cost</option>
              <option value="time">Fastest Delivery</option>
              <option value="emissions">Minimize CO₂ Emissions</option>
              <option value="utilization">Best Fleet Utilization</option>
            </select>
          </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-700/50">
              ...
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="round-trip" className="text-sm text-slate-300">
                  Include return voyage costs (doubles trip cost)
                </label>
              </div>
            </div>
          )}
